 ...
 * </p>
 * <p>
 * The <tt>JavaSource</tt> is meant for situations where one must integrate with
 ...
 * </p>
 * <p>
 * <i>Why doesn't <tt>JavaSource</tt> offer transactional guarantees?</i>
 * </p>
 * <p>
 * The problem with <tt>JavaSource</tt> and other asynchronous sources is that
 ...
 * unidirectional asynchronous interface such as JavaSource!</b> As an extension
...
public class MultiLineExecSource extends AbstractSource implements EventDrivenSource,
    ...
    @Override
    public void start() {
        logger.info("Java source starting with command:{}", command);
     ...
     */
        ...
        logger.debug("Java source started");
    }
    ...
    public void stop() {
        ...
        logger.debug("Java source with command:{} stopped. Metrics:{}", command,
                sourceCounter);
    }
    ...
    public void configure(Context context) {
        ...
        batchTimeout = context.getLong(MultiLineExecSourceConfigurationConstants.CONFIG_BATCH_TIME_OUT,
                MultiLineExecSourceConfigurationConstants.DEFAULT_BATCH_TIME_OUT);
        ...
        shell = context.getString(MultiLineExecSourceConfigurationConstants.CONFIG_SHELL, null);

        regex = Pattern.compile(context.getString(MultiLineExecSourceConfigurationConstants.CONFIG_REGEX,
                MultiLineExecSourceConfigurationConstants.DEFAULT_REGEX)).matcher("");
    ...
    private static class ExecRunnable implements Runnable {
        ...
        public ExecRunnable(String shell, String command, ChannelProcessor channelProcessor,
                            ...
                            boolean logStderr, int bufferCount, long batchTimeout, Charset charset, Matcher regex) {
                ...
                try {
                        ...
                        if (lines == null) {
                            ...
                            if(isMatch){
                                ...
                                synchronized (eventList) {
                                    ...
                                    eventList.add(EventBuilder.withBody(lines.toString().getBytes(charset)));
                    ...
                    if (reader != null) {
                        try {
                            ...
                            logger.error("Failed to close reader for java source", ex);
                        }
                    }
        ...
        public void run() {
            try {
                ...
                try {
                    ...
                    logger.error("Failed to close stderr reader for java source", ex);
                }
            }
        }
    }
